...
namespace SabberStoneCore.Config
{
	...
	public struct GameConfig
	{
		...
		public GameConfig(
			int startPlayer = START_PLAYER_DEFAULT, 
			CardClass player1HeroClass = CardClass.HUNTER,
			CardClass player2HeroClass = CardClass.MAGE,
			FormatType formatType = FormatType.FT_STANDARD,
			List<Card> player1Deck = null,
			List<Card> player2Deck = null,
			bool fillDecks = false,
			bool fillDecksPredictably = false,
			bool shuffle = true,
			bool logging = true,
			bool history = true,
			bool skipMuligan = true
		)
		{
			...
			Logging = logging;
			History = history;
			SkipMulligan = skipMuligan;

			Player1Name = String.Format(PLAYER_NAME_DEFAULT, 1);
			Player2Name = String.Format(PLAYER_NAME_DEFAULT, 2);

			Player1HeroCard = null;
			Player2HeroCard = null;
		...
		public CardClass Player1HeroClass { get; set; }

		/// <summary>
		/// Gets or sets the card class of the player with index 2.
		/// </summary>
		/// <value><see cref="CardClass"/></value>
		public CardClass Player2HeroClass { get; set; }
